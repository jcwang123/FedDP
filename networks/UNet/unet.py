...
import math
import numpy as np
from networks.UNet.layers import *
...
class ConvD(nn.Module):
    ...
    def forward(self, x, weights=None, layer_idx=None):
        ...
        if weights == None:
            ...
            weight_1, bias_1 = weights[layer_idx +
                                       '.conv1.weight'], weights[layer_idx +
                                                                 '.conv1.bias']
            weight_2, bias_2 = weights[layer_idx +
                                       '.conv2.weight'], weights[layer_idx +
                                                                 '.conv2.bias']
            weight_3, bias_3 = weights[layer_idx +
                                       '.conv3.weight'], weights[layer_idx +
                                                                 '.conv3.bias']
...
class ConvU(nn.Module):
    ...
    def forward(self, x, prev, weights=None, layer_idx=None):
        ...
        if weights == None:
            ...
            if not self.first:
                weight_1, bias_1 = weights[
                    layer_idx + '.conv1.weight'], weights[layer_idx +
                                                          '.conv1.bias']
            weight_2, bias_2 = weights[layer_idx +
                                       '.conv2.weight'], weights[layer_idx +
                                                                 '.conv2.bias']
