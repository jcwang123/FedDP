...
def linear(inputs,
    ...
    if meta_loss is not None:
        ...
        if not stop_gradient:
            grad_weight = autograd.grad(meta_loss, weight,
                                        create_graph=True)[0]

            if bias is not None:
                grad_bias = autograd.grad(meta_loss, bias,
                                          create_graph=True)[0]
            ...
            grad_weight = Variable(autograd.grad(meta_loss,
                                                 weight,
                                                 create_graph=True)[0].data,
                                   requires_grad=False)

            if bias is not None:
                grad_bias = Variable(autograd.grad(meta_loss,
                                                   bias,
                                                   create_graph=True)[0].data,
                                     requires_grad=False)
                bias_adapt = bias - grad_bias * meta_step_size
        ...
        return F.linear(inputs, weight - grad_weight * meta_step_size,
                        bias_adapt)
    else:
...
def deconv2d(inputs,
    ...
    return F.conv_transpose2d(inputs, weight, bias, stride, padding, dilation,
                              groups)
